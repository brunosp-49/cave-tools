...
const initialAspectosSocioambientaisState: NonNullable<
...
> = {
  ...
  infraestrutura_acesso: JSON.parse(
    JSON.stringify(initialInfraestruturaAcessoState)
  ),
};

const initialInfraestruturaInternaState: NonNullable<
  CaracterizacaoInterna["infraestrutura_interna"]
> = {
  nenhuma: true,
  passarela: false,
  portao: false,
...
const initialDificuldadesProgressaoInternaState: NonNullable<
  CaracterizacaoInterna["dificuldades_progressao_interna"]
> = {
  nenhuma: true,
  teto_baixo: false,
...
const initialGrupoLitologicoState: NonNullable<
  CaracterizacaoInterna["grupo_litologico"]
> = {
...
const initialCaracterizacaoInternaState: NonNullable<
...
> = {
  ...
  infraestrutura_interna: JSON.parse(
    JSON.stringify(initialInfraestruturaInternaState)
  ),
  dificuldades_progressao_interna: JSON.parse(
    JSON.stringify(initialDificuldadesProgressaoInternaState)
  ),
};

const initialDificuldadesExternasState: NonNullable<
  Cavidade["dificuldades_externas"]
> = {
  nenhuma: true,
  rastejamento: false,
  quebra_corpo: false,
...
const initialBiotaState: NonNullable<Cavidade["biota"]> = {
  // DEFINED HERE
  invertebrado: JSON.parse(JSON.stringify(initialInvertebradoState)),
  invertebrado_aquatico: JSON.parse(
    JSON.stringify(initialInvertebradoAquaticoState)
  ),
...
const initialCavidadeState: Cavidade = {
  ...
  dificuldades_externas: JSON.parse(
    JSON.stringify(initialDificuldadesExternasState)
  ),
  aspectos_socioambientais: JSON.parse(
    JSON.stringify(initialAspectosSocioambientaisState)
  ),
  caracterizacao_interna: JSON.parse(
    JSON.stringify(initialCaracterizacaoInternaState)
  ),
...
const createDefaultBiotaObjectState = (
  category: BiotaCustomObjectCategoryKey
) => {
  if (category === "invertebrado")
    return JSON.parse(JSON.stringify(initialInvertebradoState));
  if (category === "invertebrado_aquatico")
    return JSON.parse(JSON.stringify(initialInvertebradoAquaticoState));
...
const cavitySlice = createSlice({
  ...
  initialState,
  reducers: {
    setCavidadeId: (state, action: PayloadAction<string>) => {
      state.cavidade.cavidade_id = action.payload;
    },
    setStatus: (state, action: PayloadAction<"Ativo" | "Pendente">) => {
      state.cavidade.status = action.payload;
    },
      ...
      try {
        for (let i = 0; i < path.length - 1; i++) {
          ...
          if (current[key] === undefined || current[key] === null) {
            ...
            if (path[0] === "biota" && i === 0) {
              if (key === "invertebrado" || key === "invertebrado_aquatico") {
                current[key] = createDefaultBiotaObjectState(
                  key as BiotaCustomObjectCategoryKey
                );
                initialSubState = "HANDLED";
              } // Mark as handled
              else {
                initialSubState = (initialBiotaState as any)?.[key];
              }
            } else if (path[0] === "aspectos_socioambientais" && i === 0) {
              initialSubState = (initialAspectosSocioambientaisState as any)?.[
                key
              ];
            } else if (path[0] === "caracterizacao_interna" && i === 0) {
              initialSubState = (initialCaracterizacaoInternaState as any)?.[
                key
              ];
        ...
        if (path.length > 0) {
          ...
          state.error = "Error: Received empty path.";
        }
      } catch (error: any) {
        state.error =
    ...
    removeEntrada: (state, action: PayloadAction<number>) => {
      ...
      if (
        wasPrincipal &&
        state.cavidade.entradas.length > 0 &&
        !state.cavidade.entradas.some((e) => e.principal)
      ) {
    ...
    setEntradaPrincipal: (state, action: PayloadAction<number>) => {
      ...
      if (
        !state.cavidade.entradas ||
        indexToSet < 0 ||
        indexToSet >= state.cavidade.entradas.length
      ) {
    ...
    setBiotaPossui: (
      ...
      if (!state.cavidade.biota)
        state.cavidade.biota = JSON.parse(JSON.stringify(initialBiotaState));
      const currentBiota = state.cavidade.biota!;

      if (category === "peixes") currentBiota.peixes = possui;
      else if (category === "morcegos") {
        if (!currentBiota.morcegos)
          currentBiota.morcegos = JSON.parse(
            JSON.stringify(initialBiotaState.morcegos)
          );
        currentBiota.morcegos!.possui = possui;
        if (possui) {
          currentBiota.morcegos!.tipos = currentBiota.morcegos!.tipos ?? [];
          currentBiota.morcegos!.observacoes_gerais =
            currentBiota.morcegos!.observacoes_gerais ?? undefined;
        }
      } else if (
        category === "invertebrado" ||
        category === "invertebrado_aquatico"
      ) {
        const catKey = category as BiotaCustomObjectCategoryKey;
        if (!currentBiota[catKey])
          currentBiota[catKey] = createDefaultBiotaObjectState(catKey) as any;
        (currentBiota[catKey] as Invertebrado | InvertebradoAquatico).possui =
          possui;
      } else if (["anfibios", "repteis", "aves"].includes(category)) {
        const stdCatKey = category as BiotaStandardCategoryKey;
        if (!currentBiota[stdCatKey])
          currentBiota[stdCatKey] = JSON.parse(
            JSON.stringify(initialBiotaState[stdCatKey])
          );
        currentBiota[stdCatKey]!.possui = possui;
        if (possui) {
          currentBiota[stdCatKey]!.tipos = currentBiota[stdCatKey]?.tipos ?? [];
          currentBiota[stdCatKey]!.outroEnabled =
            currentBiota[stdCatKey]?.outroEnabled ?? false;
          currentBiota[stdCatKey]!.outro =
            currentBiota[stdCatKey]?.outro ?? undefined;
        }
      }
    },
    toggleBiotaTypeInArray: (
      ...
      action: PayloadAction<{
        category: BiotaStandardCategoryKey;
        type: string;
      }>
    ) => {
      ...
      if (!state.cavidade.biota?.[category]) {
        if (!state.cavidade.biota)
          state.cavidade.biota = JSON.parse(JSON.stringify(initialBiotaState));
        (state.cavidade.biota![category] as any) = JSON.parse(
          JSON.stringify(initialBiotaState[category])
        );
      }
    ...
    toggleBiotaObjectTypeFlag: (
      ...
      if (!state.cavidade.biota?.[category]) {
        if (!state.cavidade.biota)
          state.cavidade.biota = JSON.parse(JSON.stringify(initialBiotaState));
        (state.cavidade.biota![category] as any) =
          createDefaultBiotaObjectState(category);
      }
      const categoryState = state.cavidade.biota![category] as any;
      if (categoryState.possui)
        categoryState[typeKey] = !categoryState[typeKey];
    },
    toggleBiotaOutroEnabled(
      state,
      action: PayloadAction<{
        category: BiotaStandardCategoryKey | BiotaCustomObjectCategoryKey;
      }>
    ) {
      ...
      if (!state.cavidade.biota?.[category]) {
        if (!state.cavidade.biota)
          state.cavidade.biota = JSON.parse(JSON.stringify(initialBiotaState));
        (state.cavidade.biota![category] as any) =
          category === "invertebrado" || category === "invertebrado_aquatico"
            ? createDefaultBiotaObjectState(category)
    ...
    setBiotaCategoryOutro: (
      ...
      const categoryState = state.cavidade.biota![category] as any;
      if (categoryState) {
        if (categoryState.possui && categoryState.outroEnabled)
          categoryState.outro = text;
    ...
    addOrUpdateMorcegoTipo: (
      ...
      if (!state.cavidade.biota!.morcegos)
        state.cavidade.biota!.morcegos = JSON.parse(
          JSON.stringify(initialBiotaState.morcegos)
        );
      ...
      if (morcegosState.possui) {
        ...
        if (index > -1) {
          if (quantidade !== undefined)
            tiposArray[index].quantidade = quantidade;
        } else {
    ...
    setArchPalPossui: (
      ...
      if (!state.cavidade[section])
        state.cavidade[section] = JSON.parse(
          JSON.stringify(initialCavidadeState[section])
        );
    ...
    toggleArchPalTipo: (
      ...
      if (sectionState.possui && sectionState.tipos)
        (sectionState.tipos as any)[fieldName] = !(sectionState.tipos as any)[
          fieldName
        ];
    },
    toggleArchPalOutroEnabled(
      ...
      const sectionState = state.cavidade[section]!;
      if (sectionState.possui && sectionState.tipos) {
        sectionState.tipos.outroEnabled = !(
          sectionState.tipos.outroEnabled ?? false
        );
        if (!sectionState.tipos.outroEnabled)
    ...
    setSedimentoPossui: (
      ...
      if (!state.cavidade.sedimentos)
        state.cavidade.sedimentos = JSON.parse(
          JSON.stringify(initialSedimentosState)
        );
    ...
    setEspeleotemasPossui: (state, action: PayloadAction<boolean>) => {
      ...
      if (!state.cavidade.espeleotemas)
        state.cavidade.espeleotemas = { possui: possui, tipos: [] };
      else {
        state.cavidade.espeleotemas.possui = possui;
        if (possui)
          state.cavidade.espeleotemas.tipos =
            state.cavidade.espeleotemas.tipos ?? [];
    ...
    addEspeleotemaItem: (
      ...
      if (!state.cavidade.espeleotemas)
        state.cavidade.espeleotemas = { possui: true, tipos: [] };
      else state.cavidade.espeleotemas.possui = true;

      if (!state.cavidade.espeleotemas.tipos)
        state.cavidade.espeleotemas.tipos = [];
      state.cavidade.espeleotemas.tipos.push({
        ...action.payload,
    ...
    toggleUsoCavidadeOutroEnabled(state) {
      if (!state.cavidade.aspectos_socioambientais) {
        state.cavidade.aspectos_socioambientais = JSON.parse(
          JSON.stringify(initialAspectosSocioambientaisState)
        );
      }
      ...
      if (!aspectos.uso_cavidade) {
        aspectos.uso_cavidade = JSON.parse(
          JSON.stringify(initialUsoCavidadeState)
        );
      }
    ...
    toggleInfraestruturaInternaOutrosEnabled: (state) => {
      if (!state.cavidade.caracterizacao_interna) {
        state.cavidade.caracterizacao_interna = JSON.parse(
          JSON.stringify(initialCaracterizacaoInternaState)
        );
      }
      ...
      if (!ci.infraestrutura_interna) {
        ci.infraestrutura_interna = JSON.parse(
          JSON.stringify(initialInfraestruturaInternaState)
        );
      }
    ...
    setFullInfos(state, action: PayloadAction<Cavidade>) {
      ...
      const mergeDeep = (target: any, source: any, defaultSource: any) => {
        target =
          target ??
          (typeof defaultSource === "object" && defaultSource !== null
        ...
        if (typeof defaultSource === "object" && defaultSource !== null) {
          for (const key in defaultSource) {
            if (source?.[key] === undefined && target[key] === undefined) {
              // If payload doesn't have the key, and target doesn't (use default)
              ...
              defaultSource[key] !== undefined
            ) {
              if (!target[key] || typeof target[key] !== "object")
                target[key] = {};
              mergeDeep(target[key], source[key], defaultSource[key] || {});
            } else if (source?.[key] !== undefined) {
              // Primitive, array, or null from source
              target[key] = source[key];
            } else {
              // Source doesn't have key, target might already have it from initial spread
        ...
        if (typeof source === "object" && source !== null) {
          for (const key in source) {
            if (target?.[key] === undefined) {
              // Add keys from source not in default, if target also doesn't have it
              target[key] = source[key];
            }
          }
        }
    ...
    toggleDificuldadesExternasOutroEnabled: (state) => {
      if (!state.cavidade.dificuldades_externas)
        state.cavidade.dificuldades_externas = JSON.parse(
          JSON.stringify(initialDificuldadesExternasState)
        );
