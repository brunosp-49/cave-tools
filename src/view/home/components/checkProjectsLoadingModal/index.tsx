...
type InitialCheckOutcome = 'CLOSED_SILENTLY' | 'LOGGED_OFF' | 'STAY_OPEN' | 'ERROR_IN_CHECK' | 'AUTO_UPLOAD_INITIATED';
...
export const CheckProjectsModal: FC<CheckProjectsModalProps> = ({
  ...
  const downloadCompleted = Boolean(
    !networkOff && checked && (hasProjects || userHasNotProjects) && !isLoading && autoUploadStatus === 'idle'
  );
  ...
  const downloadProjects = useCallback(async () => {
    ...
    try {
      ...
      if (!users || users.length === 0 || !users[0]?.token) {
        dispatch(setModalNetworkOff(true)); throw new Error("User or token not available");
      }
      const user = users[0];
      const { data } = await api.get("/projetos/usuario/", { headers: { Authorization: `Bearer ${user.token}` }});
      if (!data.results || !Array.isArray(data.results)) throw new Error("Invalid project data format");

      if (data.results.length === 0) dispatch(setModalUserHasNotProjects(true));
      else { await createProjects(data.results); dispatch(setModalHasProjects(true)); }
  ...
  const performAutoUpload = useCallback(async (cavCount: number, projCount: number): Promise<boolean> => {
    ...
    if (cavCount > 0) {
      ...
      if (!result.success) { overallSuccess = false; errors.push(result.error || "Falha ao enviar cavidades."); }
    }
    if (overallSuccess && projCount > 0) {
      const result = await syncPendingProjects();
      if (!result.success) { overallSuccess = false; errors.push(result.error || "Falha ao enviar projetos."); }
    }
  ...
  }, [onClose, visible]); // `visible` prop from parent
  ...
  const initialCheck = useCallback(async (): Promise<InitialCheckOutcome> => {
    ...
    let serverConfirmsNoProjectsInitially = store.getState().user.userHasNotProjects; // Snapshot before check
    ...
    try {
      ...
      if (!currentUser) { await logoff(); return 'LOGGED_OFF'; }

      isCurrentlyOnline = await NetInfo.fetch().then(state => state.isConnected ?? false);
      ...
      if (isCurrentlyOnline && currentUser.last_login_date) {
        if (isTokenExpired(currentUser.last_login_date)) {
          try {
            ...
            await updateUser(String(currentUser.user_id), { token: refreshed.access, refresh_token: refreshed.refresh, last_login_date: String(new Date()) });
          } catch (refreshError) { await logoff(); return 'LOGGED_OFF'; }
      ...
      serverConfirmsNoProjectsInitially = store.getState().user.userHasNotProjects; // Re-check after potential updates
      ...
      if (outcome === 'LOGGED_OFF') {
        ...
        if (!autoUploadInitiated && autoUploadStatusRef.current === 'idle') {
          ...
          const projectsKnown = currentReduxUser.hasProjects || currentReduxUser.userHasNotProjects;

          // Conditions for silent close: everything is settled, no action/critical info for user.
          const canContinueOfflineWithProjects = !isCurrentlyOnline && currentReduxUser.hasProjects;
          const onlineAndProjectsKnownAndFine = isCurrentlyOnline && projectsKnown;

          if ((canContinueOfflineWithProjects || onlineAndProjectsKnownAndFine) &&
            ...
            console.log("[CheckProjectsModal] Conditions met for silent close.");
            onClose();
    ...
    if (visible) {
      if (!initialCheckInvoked) {
        ...
        initialCheck().then((outcome) => {
          ...
          if (outcome === 'LOGGED_OFF' || outcome === 'CLOSED_SILENTLY' || outcome === 'AUTO_UPLOAD_INITIATED') return;

          // If initialCheck decided to stay open, and no auto-upload took over,
          // the modal will render its UI based on Redux state.
          console.log("[CheckProjectsModal] initialCheck completed, outcome:", outcome, "Modal remains for UI display.");
        });
      }
    } else {
  ...
  if (autoUploadStatus === 'uploading') {
    return ( <Modal visible={true} transparent animationType="fade" onRequestClose={() => {}}>
        <View style={styles.overlay}><View style={styles.modalContainer}>
            <TextInter color={colors.white[90]} fontSize={23} style={styles.title}>Sincronizando Dados</TextInter>
            <Divider height={16} />
            <ActivityIndicator size={70} color={colors.accent[100]} />
            <Divider height={16} />
            <TextInter color={colors.dark[20]} weight="regular" style={styles.message}>Enviando dados pendentes... Por favor, aguarde.</TextInter>
        </View></View></Modal>
    );
  }
  if (autoUploadStatus === 'success') {
    return ( <Modal visible={true} transparent animationType="fade" onRequestClose={() => {}}>
        <View style={styles.overlay}><View style={styles.modalContainer}>
            ...
            <Divider height={16} /><TextInter color={colors.white[90]} fontSize={20} style={styles.title}>Sincronização Concluída</TextInter>
            <Divider height={10} /><TextInter color={colors.dark[20]} style={styles.message}>Dados enviados com sucesso!</TextInter>
        </View></View></Modal>
    );
  }
  if (autoUploadStatus === 'error') {
    return ( <Modal visible={true} transparent animationType="fade" onRequestClose={onClose}>
        <View style={styles.overlay}><View style={styles.modalContainer}>
            <Ionicons name="alert-circle-outline" size={70} color={colors.error[100]} />
            <Divider height={16} /><TextInter color={colors.white[90]} fontSize={20} style={styles.title}>Erro na Sincronização</TextInter>
            <Divider height={10} /><TextInter color={colors.dark[20]} style={styles.message}>{autoUploadError || "Falha no envio."}</TextInter>
            <Divider /><LongButton title="Fechar" onPress={onClose} mode="cancel" />
        </View></View></Modal>
    );
  }
  ...
  return (
    <Modal
      ...
      transparent
      animationType="fade"
      onRequestClose={() => { if (!isLoading && !isLoginOff) onClose(); }}
    >
      <View style={styles.overlay}>
        <View style={styles.modalContainer}>
          {isLoading ? ( // isLoading from userSlice, for the initialCheck phase
            ...
            <>
              <TextInter color={colors.white[90]} fontSize={23} style={styles.title}>Nenhum projeto encontrado</TextInter>
              <Divider height={16} /><Ionicons name="sad-outline" color={colors.accent[100]} size={70} /><Divider height={16} />
              <TextInter color={colors.dark[20]} weight="regular" style={styles.message}>Não há projetos vinculados ao seu usuário.</TextInter>
              <Divider /><LongButton title="Sair" onPress={logoff} isLoading={isLoginOff} />
            </>
          // This "Projetos Carregados" state will only show if silent close conditions were NOT met (e.g., token warning active)
            ...
            <>
              ...
              <TextInter color={colors.dark[20]} weight="regular" style={styles.message}>Seus projetos estão carregados localmente.</TextInter>
              <Divider /><LongButton title="Ok, entendi" onPress={onClose} />
            </>
          ) : checked && !hasProjects && !networkOff ? ( // Prompt for manual download
            <>
              <TextInter color={colors.white[90]} fontSize={23} style={styles.title}>Projetos não encontrados localmente</TextInter>
              <Divider height={16} /><Ionicons name="cloud-download-outline" color={colors.dark[60]} size={70} /><Divider height={16} />
              <TextInter color={colors.dark[20]} weight="regular" style={styles.message}>Deseja baixar os projetos vinculados ao seu usuário?</TextInter>
              <Divider />
            ...
            <>
              <TextInter color={colors.white[90]} fontSize={23} style={styles.title}>Você está sem internet</TextInter>
              <Divider height={16} /><Ionicons name="cloud-offline-outline" color={colors.dark[60]} size={70} /><Divider height={16} />
              <TextInter color={colors.dark[20]} weight="regular" style={styles.message}>{hasProjects ? "Trabalhando offline. Dados pendentes não puderam ser enviados." : "Conecte-se para baixar projetos e enviar dados."}</TextInter>
              <Divider />
            ...
            <>
              <TextInter color={colors.white[90]} fontSize={18} style={styles.title}>Carregando Informações...</TextInter>
