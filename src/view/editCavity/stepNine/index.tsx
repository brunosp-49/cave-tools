...
const invertebradoKeys: (keyof Omit<
  Invertebrado,
...
export const StepNine: FC<StepNineProps> = ({
  navigation,
  route,
  validationAttempted,
}) => {
  ...
  const biota =
    useSelector((state: RootState) => state.cavity.cavidade.biota) ?? {};
  ...
  const stepNineErrors = useMemo(() => {
    ...
    const errorMsgSelectOrOther =
      "Selecione pelo menos um tipo ou preencha 'Outro'.";
    const errorMsgSelectQuantity = "Selecione a quantidade.";

    const checkCustomObjectCategory = (
    ...
    const checkStandardCategory = (
      categoryKey: BiotaStandardCategoryKey,
      categoryData?: typeof biota.anfibios
    ) => {
      if (categoryData?.possui) {
        const tiposValidos =
    ...
    checkCustomObjectCategory(
      "invertebrado_aquatico",
      biota.invertebrado_aquatico
    );
    ...
    if (morcegos.possui) {
      if (
        !Array.isArray(morcegosTiposArray) ||
        morcegosTiposArray.length === 0
      ) {
        errors.morcegos_lista =
          "Adicione pelo menos um tipo de morcego se 'Possui' estiver marcado.";
      } else {
  ...
  const handleCustomObjectCategoryTypeToggle = useCallback(
    (categoryName: BiotaCustomObjectCategoryKey, typeKey: string) => {
      dispatch(
        toggleBiotaObjectTypeFlag({ category: categoryName, typeKey: typeKey })
      );
    },
    [dispatch]
  );
  ...
  const handleQuantidadeChange = useCallback(
    (feedingTypeName: BatFeedingType, value: BatQuantidadeType | "") => {
      dispatch(
        addOrUpdateMorcegoTipo({
          tipo: feedingTypeName,
          quantidade: value === "" ? undefined : value,
        })
      );
    },
    [dispatch]
      ...
      <View key={categoryKey}>
        ...
        {!!stepNineErrors[`${categoryKey}_geral`] && possui && (
          <TextInter style={styles.errorText}>
            {stepNineErrors[`${categoryKey}_geral`]}
          </TextInter>
        )}
        {possui && (
            ...
            {typeKeys.map((typeKey) => (
              <React.Fragment key={typeKey}>
                ...
                <Checkbox
                  ...
                  onChange={() =>
                    handleCustomObjectCategoryTypeToggle(categoryKey, typeKey)
                  }
                />
              </React.Fragment>
            ))}
            ...
            {types.map((type) => (
              <React.Fragment key={type}>
                ...
                <Checkbox
                  ...
                  onChange={() =>
                    handleStandardCategoryTypeToggle(categoryKey, type)
                  }
                />
              </React.Fragment>
            ))}
        ...
        {isChecked && (
          <>
            ...
            <Select
              ...
              onChangeText={(obj) => {
                const value =
                  obj.id === "" ? "" : (obj.id as BatQuantidadeType);
                handleQuantidadeChange(
                  feedingKey,
                  value as BatQuantidadeType | ""
                );
              }}
              value={
  ...
  return (
    ...
    >
      ...
      <Divider />
      {renderCustomObjectCategorySection(
        "invertebrado",
        "Invertebrado",
        invertebradoKeys,
        invertebradoLabels
      ...
      {renderCustomObjectCategorySection(
        ...
        invertebradoAquaticoKeys,
        invertebradoAquaticoLabels
      )}
      {renderStandardCategorySection("anfibios", "Anfíbio", [
        "Sapo",
        "Rã",
        "Perereca",
      ])}
      {renderStandardCategorySection("repteis", "Réptil", [
        "Serpente",
        "Lagarto",
      ])}
      {renderStandardCategorySection("aves", "Ave", [
        "Urubu",
        "Gavião",
        "Arara azul",
        "Arara vermelha",
